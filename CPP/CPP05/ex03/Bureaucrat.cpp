...
void	Bureaucrat::signForm(AForm& a) {
	try {
		a.beSigned(*this);
		std::cout << this->name_ << " signed " << a.getName() << "\n";
	}
	catch (std::exception& e) {
		std::cout << this->name_ << " couldn't sign " << a.getName() << ": " << e.what() << "\n";
	}
}
...
void	Bureaucrat::executeForm(AForm const& a) {
	try {
		a.execute(*this);
		std::cout << this->name_ << " executed " << a.getName() << "\n";
	}
	catch (std::exception& e) {
		std::cout << this->name_ << " couldn't execute " << a.getName() << " because " << e.what() << "\n";
	}
}
